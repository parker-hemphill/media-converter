...
handbrake_options=" --markers  -O --encoder x265 --encopts vbv-maxrate=25000:vbv-bufsize=31250 --crop 0:0:0:0"
...
if [ "${input: -4}" == ".mkv" ]; then
   $ffmpeg -threads 6 -i "$input" -codec copy "${input%.*}.mp4"
   if [[ $? -eq 0 ]]
   then
     rm "$input"
     exit 0
...
if [[ ${PIPESTATUS[0]} -eq 0 ]]; then
 ...
 if [[ ! -f $log ]]; then
   ...
   echo "[ Movie ] [$(date "+%D %H:%M")] [${input_size}|${output_size}] $input" >> $log
 else
   echo "[ Movie ] [$(date "+%D %H:%M")] [${input_size}|${output_size}] $input" >> $log
 fi
else
...
count=0
rm "$media_convert/*.srt" > /dev/null 2>&1
